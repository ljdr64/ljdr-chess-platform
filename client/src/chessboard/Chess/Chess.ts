...
import { ChessEngine, MoveValidationError } from './Engine/ChessEngine.ts';
import { ChessBoard } from './Board/ChessBoard.ts';
import { SquareClickMode, SquareEffect } from './Board/Types/index.ts';
import { Store, StoreKey } from '@ChessPlatform/chessboard/Services/Store.ts';
import { Converter } from './Utils/Converter.ts';
import { Logger } from '@ChessPlatform/chessboard/Services/Logger.ts';
import { Bot, BotAttributes } from './Bot/index.ts';
...
export class Chess {
    ...
    constructor() {
        ...
        this.engine = new ChessEngine((log: string) => {
            engineLogger.save(log);
        });
        const boardLogger = new Logger('src/Chess/Board/ChessBoard.ts');
        this.board = new ChessBoard((log: string) => {
            boardLogger.save(log);
        });
    ...
    public checkAndLoadGameFromCache(): void {
        ...
        if (Store.isExist(StoreKey.LastBoard)) {
            ...
            this.createGame(Store.load(StoreKey.LastBoard) as JsonNotation);
    ...
    public createGame(
        ...
        Store.save(StoreKey.LastBoard, this.getGameAsJsonNotation());
        Store.save(StoreKey.LastCreatedBoard, this.getGameAsFenNotation());
    ...
    public createPiece(
        ...
        Store.save(StoreKey.LastBoard, this.engine.getGameAsJsonNotation());
        Store.save(
    ...
    private handleOnPieceMoved(squareId: Square): void {
        ...
        if (this.board.getLockedColor()) this.board.lock();
    ...
    public async playMove(from: Square, to: Square): Promise<void> {
        ...
        try {
            if (this._isNonDomMove) {
                ...
                this._isLastNonDomMovePromotion =
        ...
        this._bot.getMove(this.engine.getGameAsFenNotation()).then((move) => {
            ...
            document.dispatchEvent(
                new CustomEvent(ChessEvent.onPieceMovedByOpponent, {
                    detail: {
                        from: move[move.length - 1].from,
                        to: move[move.length - 1].to,
                    },
                })
            );
        });
