...
export class ChessEngine {
    ...
    public checkAndFindMoveType(from: Square, to: Square): MoveType | null {
        ...
        this.logger?.save(
            `Move type is not found because the given move-ts-${to}-te- is not in the current moves-ts-${JSON.stringify(
    ...
    public getMoves(
        ...
        if (
            ...
            !BoardQuerier.isSquareSelectable(square)
        ) {
            this.logger?.save(
    ...
    private _doCastling(): void {
        ...
        const castlingSide: CastlingSide =
            Number(this.playedFrom) > Number(this.playedTo)
                ...
                : CastlingSide.Short;
        this.logger?.save(
            `Castling type determined-ts-${castlingSide}-te- on engine`
        );
    ...
    private checkEnPassant(): void {
        if (BoardQuerier.getEnPassant() !== null) {
            ...
            this.logger?.save(
                'undone en passant move removed from fen notation'
            );
        }
    ...
    private checkCastling(): void {
        ...
        if (
            ...
            this.logger?.save(
                `Castling moves-ts-${BoardQuerier.getCastling()}-te- are already disabled`
            );
            return;
        }
