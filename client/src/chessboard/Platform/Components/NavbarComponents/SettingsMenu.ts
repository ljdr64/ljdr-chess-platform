...
import { SettingsMenuOperation } from '@ChessPlatform/chessboard/Platform/Types';
import { SETTINGS_MENU_ID } from '@ChessPlatform/chessboard/Platform/Consts';
...
} from '@ChessPlatform/chessboard/Chess/Board/Types';
import { DEFAULT_CONFIG as DEFAULT_SETTINGS_CHESSBOARD } from '@ChessPlatform/chessboard/Chess/Board/ChessBoard';
import {
    LogConsole,
...
import { Formatter } from '@ChessPlatform/chessboard/Platform/Utils/Formatter';
 ...
 */
type SettingValue =
    Settings[SettingsConfigOperation][keyof Settings[SettingsConfigOperation]];
...
export class SettingsMenu extends NavbarComponent {
    ...
    constructor(chess: Chess, ...components: Component[]) {
        ...
        for (const component of components) {
            if (!(component instanceof Component))
                throw new Error(
                    'The given object is not an instance of Component.'
                );
    ...
    private loadLocalStorage(): void {
        ...
        if (!settings) {
            Store.save(StoreKey.Settings, this.getDefaultSettings());
        } else {
            for (const config in settings) {
                for (const [settingKey, settingValue] of Object.entries(
                    settings[config as SettingsConfigOperation]
                )) {
                    this.saveSetting(
    ...
    protected renderComponent(): void {
         ...
         */
        const getCompleteEnumValues = <
            T extends SettingsConfigOperation,
        ...
        >(
            ...
            switch (settingKey) {
                case (currentSettingsKeyProxy as ChessBoardConfig)
                    .animationSpeed:
                    return Object.values(AnimationSpeed);
                case (currentSettingsKeyProxy as NotationMenuConfig)
                    .notationStyle:
        ...
        this.loadHTML(
            ...
            <div id="settings-body">
                ${Object.entries(currentSettings)
                    .map(([settingConfigOperation, settingKeyValuePair]) => {
                        return `
                    <fieldset>
                        <legend>${Formatter.camelCaseToTitleCase(
                            settingConfigOperation
                                .replace('Change', '')
                                .replace('Setting', '')
                        )}</legend>
                        ${Object.entries(settingKeyValuePair)
    ...
    private saveSetting<
        T extends SettingsConfigOperation,
        K extends keyof Settings[T]
    >(configOperation: T, settingKey: K, settingValue: Settings[T][K]): void {
        ...
        switch (configOperation) {
            ...
            case SettingsMenuOperation.ChangeNotationMenuSetting:
                (
                    this.getComponentInstanceByType(
                        NotationMenu
                    ) as NotationMenu
                ).setConfig({
                    [settingKey]: settingValue,
    ...
    public handleOperation(
        ...
        if (menuItem.getAttribute('type') === 'checkbox') {
            ...
            settingValue = Formatter.titleCaseToCamelCase(
