...
import { Logger, LoggerEvent } from '@ChessPlatform/chessboard/Services/Logger';
import { NavbarComponent } from './NavbarComponent';
import { LOG_CONSOLE_ID } from '@ChessPlatform/chessboard/Platform/Consts';
import { debounce } from '@ChessPlatform/chessboard/Utils/Timing';
...
export class LogConsole extends NavbarComponent {
    ...
    protected renderComponent(): void {
        ...
        if (!logConsoleBody || !downButton) return;

        logConsoleBody!.addEventListener(
            'scroll',
            debounce(() => {
                if (!logConsoleBody || !downButton) return;

                const isNearBottom =
                    logConsoleBody.scrollHeight - logConsoleBody.scrollTop <=
                    logConsoleBody.clientHeight + LOG_DOWN_ACTIVATION_THRESHOLD;
                if (isNearBottom) {
    ...
    private _createLogMessages(
        ...
        const lastLogListElement: HTMLElement | null =
            logListElement.querySelector('li:last-child');
        ...
        for (const log of logs) {
            ...
            logElement.innerHTML += log.message.replace(
                /\b(\w+)\s*-ts-(.*?)-te-/g,
                (match, prevWord, content) =>
                    `${generateTooltipToggle(prevWord, content)}`
            );
    ...
    private _createLogMessagesEventListeners(): void {
        ...
        // Tooltips
        const squares: NodeListOf<HTMLElement> | null =
    ...
    private _getDownButton(): HTMLElement | null {
        return document.querySelector(
            `[data-menu-operation="${LogConsoleOperation.Down}"]`
        );
    }
    ...
    public down(): void {
        ...
        if (logConsoleBody && logConsoleList) {
            logConsoleBody.scrollTo({
                top: logConsoleBody.scrollHeight,
                behavior: 'smooth',
            });
            logConsoleList.scrollTo({
                top: logConsoleList.scrollHeight,
                behavior: 'smooth',
            });
